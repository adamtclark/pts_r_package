...
filterout_det<-particleFilterLL(y, pars=parseparam0(parsest_det[,1]), detfun = detfun0,
                                dotraceback = TRUE)
#based on EDM
filterout_edm<-particleFilterLL(y, pars=parseparam0(parsest_edm[1,1]), detfun = EDMfun0, edmdat = list(E=2, theta=exp(parsest_edm[2,1])),
                                dotraceback = TRUE)
...
filterout_edm_true<-particleFilterLL(y, pars=parseparamold(ptrue), detfun = EDMfun0, edmdat = list(E=2, theta=sout$theta[which.max(sout$rho)]),
                                dotraceback = TRUE)
...
save(list = c("simdat", "parslst", "optdat", "filterdat"), file = paste("datout/mcmcout_", commArgin, "_noobs.rda", sep=""), version=2)
