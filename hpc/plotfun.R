plotfun<-function(plotvar, byvar, summarydat, cutlst, mrowpar, collst, cutoff=0.3, doci=TRUE, ...) {
  if(plotvar=="proc") {
  ...
  for(i in 1:length(bylist)) {
    ...
    matplot(xv[ps],
            pmat,
            xlab="", ylab="",
            type="p", main=bylist[i],
            log="xy",pch=1,cex=0.5,
            col=cuse, ...)
    ...
    if(plotvar!="det") {
      ...
      abline(v=c(0.1, 0.5), lty=3)
    }
    
    if(plotvar%in%c("obs", "proc")) {
      pscut<-which(xv[ps]<cutoff)
      e2<-pmax(0, 1-colMeans((xv[ps][pscut]-pmat[pscut,])^2,na.rm=T)/mean((xv[ps][pscut]-mean(xv[ps][pscut]))^2,na.rm=T))
      legend("topleft", legend = round(e2, 2), col=adjustcolor(cuse,alpha.f = 2), pch=1, bty="n")
    }
