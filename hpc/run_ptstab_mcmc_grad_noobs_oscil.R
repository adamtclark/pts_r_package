...
#create priors
p0_simulate<-list(c(-4, 0),c(-4, 0))
minvUSE_simulate<-unlist(lapply(p0_simulate, function(x) x[1]))
maxvUSE_simulate<-unlist(lapply(p0_simulate, function(x) x[2]))

sampler_fun_USE_simulate<-function(x) sampler_fun0(n = 1, minv = minvUSE_simulate, maxv=maxvUSE_simulate)

p0<-list(c(-4, 0))
minvUSE<-unlist(lapply(p0, function(x) x[1]))
maxvUSE<-unlist(lapply(p0, function(x) x[2]))

p0_edm<-list(c(-4, 0), c(-5, 2))
...
while(sum(y>0)<=(length(y)/20)) { # want at least 5% nonzero values
  pars_sim<-parseparam0(unname(sampler_fun_USE_simulate()))
...
filterout_det<-particleFilterLL(y, pars=parseparam0(parsest_det[,1]), detfun = detfun0,
                                dotraceback = TRUE)
#based on EDM
filterout_edm<-particleFilterLL(y, pars=parseparam0(parsest_edm[1,1]), detfun = EDMfun0, edmdat = list(E=Euse, theta=exp(parsest_edm[2,1])),
                                dotraceback = TRUE)
#based on true values
filterout_true<-particleFilterLL(y, pars=parseparamold(ptrue), detfun = detfun0,
                                dotraceback = TRUE)
#based on EDM, with correct values
sout<-s_map(y, E=Euse,silent = TRUE)
filterout_edm_true<-particleFilterLL(y, pars=parseparamold(ptrue), detfun = EDMfun0, edmdat = list(E=Euse, theta=sout$theta[which.max(sout$rho)]),
                                dotraceback = TRUE)
...
save(list = c("simdat", "parslst", "optdat", "filterdat"), file = paste("datout/mcmcout_", commArgin, "_oscil_noobs.rda", sep=""), version=2)
