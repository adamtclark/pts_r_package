...
## Simulate data
#sample from priors
pars0<-pars_true<-list(obs=c(log(0.01), log(0.2)),
                       proc=c(log(0.01), log(1.5)),
...
## Run filter
ptrue<-unname(unlist(pars_sim)[1:4])
...
save(list = c("simdat", "parslst", "optdat", "filterdat"), file = paste("datout/mcmcout_", commArgin, "_full_oscil_4E.rda", sep=""), version=2)
