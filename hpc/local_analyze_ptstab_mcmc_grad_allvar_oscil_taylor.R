...
p0<-list(c(-5, -2), c(-4, 0.5), c(-5, 0), c(-0.2, 1.1))
minvUSE<-unlist(lapply(p0, function(x) x[1]))
maxvUSE<-unlist(lapply(p0, function(x) x[2]))

p0_edm<-list(c(-5, -2), c(-4, 0.5), c(-5, 0), c(-0.2, 1.1), c(-5, 2))
...
if(FALSE) {
  ...
  for(ifl in 1:length(flst)) {
    ...
    summarydat$obs1[ifl]<-exp(pars_true[2])
    summarydat$proc0[ifl]<-exp(pars_true[3])
    summarydat$proc1[ifl]<-exp(pars_true[4])

    summarydat$det_obs_mu0[ifl]<-exp(parslst$parsest_det[1,1])
    summarydat$det_obs_mu1[ifl]<-exp(parslst$parsest_det[2,1])
    summarydat$det_proc_mu0[ifl]<-exp(parslst$parsest_det[3,1])
    summarydat$det_proc_mu1[ifl]<-exp(parslst$parsest_det[4,1])
    summarydat$edm_obs_mu0[ifl]<-exp(parslst$parsest_edm[1,1])
    summarydat$edm_obs_mu1[ifl]<-exp(parslst$parsest_edm[2,1])
    summarydat$edm_proc_mu0[ifl]<-exp(parslst$parsest_edm[3,1])
    summarydat$edm_proc_mu1[ifl]<-exp(parslst$parsest_edm[4,1])
    ...
    stmp<-s_map(datout$obs,silent = TRUE,E=Euse)
  ...
  write.csv(summarydat, "datout/summarydat_oscil_taylor.csv", row.names = FALSE)
} else {
  summarydat<-read.csv("datout/summarydat_oscil_taylor.csv")
}
...
#tmp
plot(obs0~det_obs_mu0, summarydat[summarydat$summed_proc_error<=cutoff & summarydat$gelmandet<=1.1,], log="xy"); abline(a=0, b=1, lty=3)
plot(obs0~edm_obs_mu0, summarydat[summarydat$summed_proc_error<=cutoff & summarydat$gelmanedm<=1.1,], log="xy"); abline(a=0, b=1, lty=3)

plot(obs1~det_obs_mu1, summarydat[summarydat$summed_proc_error<=cutoff & summarydat$gelmandet<=1.1,], log="xy"); abline(a=0, b=1, lty=3)
plot(obs1~edm_obs_mu1, summarydat[summarydat$summed_proc_error<=cutoff & summarydat$gelmanedm<=1.1,], log="xy"); abline(a=0, b=1, lty=3)

plot(proc0~det_proc_mu0, summarydat[summarydat$summed_obs_error<=cutoff & summarydat$gelmandet<=1.1,], log="xy"); abline(a=0, b=1, lty=3)
plot(proc0~edm_proc_mu0, summarydat[summarydat$summed_obs_error<=cutoff & summarydat$gelmanedm<=1.1,], log="xy"); abline(a=0, b=1, lty=3)

plot(proc1~det_proc_mu1, summarydat[summarydat$summed_obs_error<=cutoff & summarydat$gelmandet<=1.1,], log="xy"); abline(a=0, b=1, lty=3)
plot(proc1~edm_proc_mu1, summarydat[summarydat$summed_obs_error<=cutoff & summarydat$gelmanedm<=1.1,], log="xy"); abline(a=0, b=1, lty=3)

summary(lm(log(proc1)~log(det_proc_mu1), summarydat[summarydat$summed_obs_error<=cutoff & summarydat$gelmandet<=1.1,]))
