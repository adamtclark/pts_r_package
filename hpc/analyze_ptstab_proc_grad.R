...
colrates<-matrix(nrow=length(prcsq), ncol=4)
morrates<-matrix(nrow=length(prcsq), ncol=4)
...
if(FALSE) {
  ...
  save.image("summarydata/saved_summary.rda")
} else {
  load("summarydata/saved_summary.rda")
}
...
qtlout_col<-array(dim=c(5,4,length(prclst)))
qtlout_mor<-array(dim=c(5,4,length(prclst)))
